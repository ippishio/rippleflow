...
class HceCardEmulationApduService : HostApduService() {
    ...
    override fun onStartCommand(intent: Intent, flags: Int, startId: Int): Int {
        if (intent.hasExtra(NFC_NDEF_KEY)) {
            ndefUri =
                NdefMessage(createTextRecord(Locale.getDefault().language, intent.getStringExtra(
                    NFC_NDEF_KEY
                ) ?: "", ndefId))
    ...
    private fun onDataShared() {
        val intent = Intent(BottomSheetNFCFragment.NFC_BROADCAST).apply {
            putExtra(BottomSheetNFCFragment.NFC_EXTRA_NDEF_SENT, true)
        }
